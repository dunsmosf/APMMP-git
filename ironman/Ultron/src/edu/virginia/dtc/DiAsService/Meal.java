...
public class Meal {
	...
	public void dump() {
		...
		Debug.i(TAG, FUNC_TAG, "Meal> time_announce="+time_announce+", time="+time+", meal_size_grams="+meal_size_grams+", SMBG="+SMBG);
		Debug.i(TAG, FUNC_TAG, "Meal> meal_screen_bolus="+meal_screen_bolus+", type="+type+", treated="+treated+", active="+active+", approved="+approved);
	}
...
//	}
			...
			if (creq.moveToFirst()) {
				do{
					...
					if ((active || approved) && extended_bolus_insulin_rem > 0.05) {
                        ...
                        trIntent.setClass(context, edu.virginia.dtc.DiAsService.Alerts.class);
	...
	public boolean readActive() {
		final String FUNC_TAG = "readActive";
	...
	// Update the local copy of the meal record
	public boolean read() {
	...
	public boolean update() {
	    ...
	    if (_id != -1) {		// Valid _id field
		    try {
				...
				Debug.i(TAG, FUNC_TAG, "Meal > update > meal record not updated, error"+e.getMessage());
		    }
	    }
	    else {
			Debug.i(TAG, FUNC_TAG, "Meal > update > meal record not updated");
	    }
	    return return_value;
	}
