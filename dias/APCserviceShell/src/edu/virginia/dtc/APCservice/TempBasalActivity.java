...
public class TempBasalActivity extends Activity{
	...
	public static final String TAG = "TempBasalActivity";
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		if (getIntent().getIntExtra("command", TempBasal.TEMP_BASAL_CANCEL) == TempBasal.TEMP_BASAL_START) {
			...
			percentSpinner.setSelection(10);
	...
	************************************************************************************/
	...
	private void startTempBasal() 
	{
	    ...
	    values.put("owner", TempBasal.TEMP_BASAL_OWNER_APCSERVICE);
		Bundle b = new Bundle();
		try 
	    {
	    	Uri uri = getContentResolver().insert(Biometrics.TEMP_BASAL_URI, values);
 	    	b.putString("description", "DiAsMain > startTempBasal, start_time= "+time);
 	    	Event.addEvent(getApplicationContext(), Event.EVENT_TEMP_BASAL_STARTED, Event.makeJsonString(b), Event.SET_LOG);
	    }
	    catch (Exception e) 
	    {
 	    	b.putString("description", "DiAsMain > startTempBasal failed, start_time= "+time+", "+e.getMessage());
 	    	Event.addEvent(getApplicationContext(), Event.EVENT_TEMP_BASAL_STARTED, Event.makeJsonString(b), Event.SET_LOG);
