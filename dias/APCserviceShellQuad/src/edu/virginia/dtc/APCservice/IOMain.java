...
import com.joptimizer.AllTests;
...
//                	Toast.makeText(getApplicationContext(), TAG+" > APC_SERVICE_CMD_NULL", Toast.LENGTH_LONG).show();
				...
				case APC_SERVICE_CMD_CALCULATE_STATE:
					...
					//****************
					// 1) Calculate Insulin here.
					// 		Insulin is returned in:
					//			differential_basal_rate (Units/hour in addition to basal rate.  DiAs will convert to appropriate 5 minute micro-bolus)
					//			recommended_bolus (Units: this can be used do deliver a correction bolus)
					//
					// 2) We have inserted a call to the TestSuite for jOptimizer and display the result as an Event which appears on the DiAs
					//		screen and is also logged into the database.  The TestSuite can be modified in order to measure the performance of
					//		the system.
					//****************
					// BEGIN jOptimizer TESTS
					...
					if(result.wasSuccessful()){
						Log.i(JOPTIMIZER_LOGTAG, "JOptimizer is working properly!");
	            		Bundle b1 = new Bundle();
	            		b1.putString("description", "JOptimizer is working properly!");
                		Event.addEvent(getApplicationContext(), Event.EVENT_SYSTEM_IO_TEST, Event.makeJsonString(b1), Event.SET_POPUP);
						Toast.makeText(getApplicationContext(), "JOptimizer is working properly!", Toast.LENGTH_LONG).show();
					}else{
						Log.i(JOPTIMIZER_LOGTAG, "JOptimizer is NOT working properly!");
	            		Bundle b2 = new Bundle();
	            		b2.putString("description", "JOptimizer is NOT working properly!");
                		Event.addEvent(getApplicationContext(), Event.EVENT_SYSTEM_IO_TEST, Event.makeJsonString(b2), Event.SET_POPUP);
						Toast.makeText(getApplicationContext(), "JOptimizer is NOT working properly!", Toast.LENGTH_LONG).show();
					}
					// END jOptimizer TESTS
