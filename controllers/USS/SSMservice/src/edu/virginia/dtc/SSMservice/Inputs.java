...
public class Inputs {
  ...
  public void reconstruct (long time,Context callingContext) {
      ...
      //  convert CGM_array CGMCal_array to string
	  StringBuffer CGM_array_string = new StringBuffer();
	  StringBuffer CGMCal_array_string = new StringBuffer();
      for (i=0;i<CGM_array.length;i++){  
		  CGM_array_string.append(String.format("%.1f", CGM_array[i])+" ");
		  CGMCal_array_string.append(String.format("%.1f", CGMCal_array[i])+" ");	 
	  }
	  
	  
	  String a = CGM_array_string.toString();
	  String b = CGMCal_array_string.toString();
	  ...
	  //convert CGM_clean to string 
	  StringBuffer CGM_clean_string = new StringBuffer();
	  
	  for (i=0;i<CGM_clean.length;i++){	  
		  CGM_clean_string.append(String.format("%.1f", CGM_clean[i])+" "); 
	  }  
	  String c = CGM_clean_string.toString();

	
      debug_message(TAG, "CGM_array_string:"+a);
      debug_message(TAG, "CGMCal_array_string:"+b);
