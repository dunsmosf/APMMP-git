...
public class Subject {
	...
	public boolean read(long time, Context calling_context) {
		...
		indices = subject_data.subjectCF.find(">", -1, "<=", timeNowSecs);			// Find the list of indices <= time in minutes since today at 00:00
		if (indices == null) {
		...
		indices = subject_data.subjectCR.find(">", -1, "<=", timeNowSecs);			// Find the list of indices <= time in minutes since today at 00:00
		if (indices == null) {
		...
		indices = subject_data.subjectBasal.find(">", -1, "<=", timeNowSecs);		// Find the list of indices <= time in minutes since today at 00:00
		if (indices == null) {
	...
	private DiAsSubjectData readDiAsSubjectData(Context calling_context) {
		...
		if (c.moveToLast()) {
			...
			int enableIOtestInt = c.getInt(c.getColumnIndex("enableIOtest"));
			if (enableIOtestInt == 1) {
				enableIOtest = true;
				subject_data.enableIOtest = true;
				Log.i(TAG, "readDiAsSubjectData > enableIOtest=true");
			}
			else if (enableIOtestInt == 0) {
				enableIOtest = false;
				subject_data.enableIOtest = false;
				Log.i(TAG, "readDiAsSubjectData > enableIOtest=false");
			}
			else {
				enableIOtest = false;
				subject_data.enableIOtest = false;
				Log.i(TAG, "readDiAsSubjectData > enableIOtestInt=="+enableIOtestInt);
			}
