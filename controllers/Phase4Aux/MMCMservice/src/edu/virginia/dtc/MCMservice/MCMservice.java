        ...
        public void handleMessage(Message msg) {
			...
			switch (msg.what) 
            {
	            ...
	            case MCM_OCAD_MEAL_ACTIVITY_CALCULATE:
	            	...
	            	optcorr_bolus_calculation = new OptCORR_processing(optcorr_param,
												            			Tvec_meal,
												            			Tvec_cgm1,
												            			Tvec_insulin_rate1,
												            			currentTimeseconds,
...
//		debug_message(TAG,"CGM > c.getCount="+c.getCount());
		...
		if (c.getCount()<=144) {
			...
			do{
				...
				if (cgm1_value>=39.0 && cgm1_value<=401.0) {
					...
					// time in seconds
					Tvec_cgm1.put(c.getLong(c.getColumnIndex("time"))/60, cgm1_value);
					//return_value = true;
				}
			} while (c.moveToNext());
	        }						
			...
			c.moveToPosition(c.getCount()-144);
				do{
					// Fetch the cgm1 and cgm2 values so that they can be screened for validity
					cgm1_value = (double)c.getDouble(c.getColumnIndex("cgm"));
					// Make sure that cgm1_value is in the range of validity
		...
		debug_message(TAG,"CGM length " + Tvec_cgm1.count());
		
		
		// Fetch INSULIN data
		//c=getContentResolver().query(INSULIN_URI, null, last_Tvec_insulin_rate1_time_secs.toString(), null, null);
		c=getContentResolver().query(Biometrics.INSULIN_URI, null, null, null, null);
		...
		if (c.getCount()<=144) {
			...
			do {
				...
				Tvec_insulin_rate1.put(c.getLong(c.getColumnIndex("deliv_time"))/60, insulin_value);
				}
			 }	
			while(c.moveToNext());
		    }
		...
		debug_message(TAG,"Insulin length " + Tvec_insulin_rate1.count());


		
		// Fetch MEAL data
		//c=getContentResolver().query(MEAL_URI, null, Time.toString(), null, null);
		c=getContentResolver().query(Biometrics.MEAL_URI, null, null, null, null);
...
//		debug_message(TAG,"CGM > c.getCount="+c.getCount());
		...
		if (c.moveToFirst()) {
			do{
				if ( (currentTimeseconds -c.getLong(c.getColumnIndex("time")))<=43200 && (currentTimeseconds -c.getLong(c.getColumnIndex("time")))>=1800 ) {
				meal_value = (double)c.getDouble(c.getColumnIndex("meal_size_grams"));
				// Make sure that cgm1_value is in the range of validity
					Tvec_meal.put(c.getLong(c.getColumnIndex("time"))/60, meal_value);
				}	
			} while (c.moveToNext());
		...
		debug_message(TAG,"Meal length " + Tvec_meal.count());
